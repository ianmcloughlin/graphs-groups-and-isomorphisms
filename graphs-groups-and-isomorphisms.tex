...
\section{Graphs} 
  ...
  \begin{figure}
    \centering
    \begin{tikzpicture}[every node/.style={circle,fill=atuteal}, every edge/.style={draw=atugreen,thick}]
      \draw
        (0, 0) node (a) {$a$}
        (2, 0) node (b) {$b$}
        (4, 0) node (c) {$c$}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[every node/.style={circle}, every edge/.style={thick}]
      \draw
        ...
        ++ (  0:2) node[fill=atuteal]   (2) {$2$}
        ++ (300:2) node[fill=atuteal]   (3) {$3$}
        ++ (240:2) node[fill=atuteal]   (4) {$4$}
        ++ (180:2) node[fill=atuorange] (5) {$5$}
        (0) edge[draw=atunavy]  (1)
        (1) edge[draw=black]    (2)
        (2) edge[draw=atugreen] (3)
        (3) edge[draw=atugreen] (4)
        (4) edge[draw=black]    (5)
    ...
    \begin{tikzpicture}
      \begin{scope}[every node/.style={circle,fill=atunavy}, every edge/.style={draw=black,thick}]
        \draw
          ...
          (e) edge (f);
      \end{scope}
      \begin{scope}[every node/.style={circle,fill=atugreen}, every edge/.style={draw=black,thick}, xshift=50mm]
        \draw
          (0,0) node (a) {}
          (0,2) node (b) {}
          (2,0) node (c) {}
